...
class QuestionMedController extends GetxController {
  ...
  Future<void> loadAvailableMYears(String selectedYear, String selectedSem, String selectedMod) async {
    try {
      availableYears.clear();

      // Step 1: Load Anatomie JSON to find the module
      final String MedJsonString = await rootBundle.loadString('assets/resources/annee1S1/Medecine.json');
      final Map<String, dynamic> MedData = json.decode(MedJsonString);

      if (MedData['yearsMed'] is! List) {
        throw Exception('Invalid format: years section is missing or not a list.');
      }

      final List<dynamic> yearsData = MedData['yearsMed'];
  ...
  Future<void> loadQuestionsMForYear(String selectedYear, String selectedSem, String selectedMod, String year) async {
    try {
      // Step 1: Load Anatomie JSON to find the module's JSON file path
      final String anatomieJsonString = await rootBundle.loadString('assets/resources/annee1S1/Medecine.json');
      final Map<String, dynamic> anatomieData = json.decode(anatomieJsonString);

      final List<dynamic> yearsData = anatomieData['yearsMed'];
